...
LRESULT CALLBACK EnergyStored(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	switch (message) {
		...
		case WM_INITDIALOG: {
 		  ...
 		  shidi.dwFlags = SHIDIF_DONEBUTTON | SHIDIF_SIZEDLGFULLSCREEN;
			...
			TabItem.pszText = TEXT("Capacitor");
			// Insert new tab
			TabCtrl_InsertItem(TabHandle, 1, &TabItem);
			// Define new tab
			TabItem.pszText = TEXT("Inductor");
			// Insert new tab
      ...
      switch (tem->code) {
        case TCN_SELCHANGE: {
          // Hide the current child
					ShowWindow(hWndEnergyStoredChild, SW_HIDE);
          ...
          switch (whatTab) {
            ...
            case 2: {
              // Capacitive tab
              hWndEnergyStoredChild = hWndEnergyStoredInductor;
						break; }
          }
					ShowWindow(hWndEnergyStoredChild, SW_SHOW);
