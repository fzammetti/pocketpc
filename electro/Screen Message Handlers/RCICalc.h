...
LRESULT CALLBACK RCICalc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	switch (message) {
		...
		case WM_INITDIALOG: {
 		  ...
 		  shidi.dwFlags = SHIDIF_DONEBUTTON | SHIDIF_SIZEDLGFULLSCREEN;
			...
			TabItem.pszText = TEXT("Resistance");
			// Insert new tab
			TabCtrl_InsertItem(TabHandle, 1, &TabItem);
			// Define new tab
			TabItem.pszText = TEXT("Capacitance");
			// Insert new tab
			TabCtrl_InsertItem(TabHandle, 2, &TabItem);
			// Define new tab
			TabItem.pszText = TEXT("Inductance");
			// Insert new tab
			TabCtrl_InsertItem(TabHandle, 2, &TabItem);
			return true;
		break; }
      ...
      switch (tem->code) {
        ...
        case TCN_SELCHANGE: {
          // Hide the current child
					ShowWindow(hWndRCICalcChild, SW_HIDE);
          ...
          switch (whatTab) {
            ...
            case 2: {
              // Capacitance tab
              hWndRCICalcChild = hWndRCICalcCapacitance;
						break; }
            case 3: {
              // Inductance tab
              hWndRCICalcChild = hWndRCICalcInductance;
						break; }
          }
					ShowWindow(hWndRCICalcChild, SW_SHOW);
