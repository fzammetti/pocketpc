...
LRESULT CALLBACK Timer555(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	switch (message) {
		...
		case WM_INITDIALOG: {
 		  ...
 		  shidi.dwFlags = SHIDIF_DONEBUTTON | SHIDIF_SIZEDLGFULLSCREEN;
			...
			TabItem.pszText = TEXT("Monostable");
			// Insert new tab
			TabCtrl_InsertItem(TabHandle, 1, &TabItem);
			// Define new tab
			TabItem.pszText = TEXT("Astable");
			// Insert new tab
      ...
      switch (tem->code) {
        case TCN_SELCHANGE: {
          // Hide the current child
					ShowWindow(hWndTimer555Child, SW_HIDE);
          ...
          switch (whatTab) {
            ...
            case 2: {
              // Astable tab
              hWndTimer555Child = hWndTimer555Astable;
						break; }
          }
					ShowWindow(hWndTimer555Child, SW_SHOW);
